...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd := utils.GoTool("kubectl", "create", "ns", namespace)
		_, err := utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to create namespace")

		By("labeling the namespace to enforce the restricted security policy")
		cmd = utils.GoTool("kubectl", "label", "--overwrite", "ns", namespace,
	...
	AfterAll(func() {
		...
		cmd := utils.GoTool("kubectl", "delete", "pod", "curl-metrics", "-n", namespace)
		_, _ = utils.Run(cmd)
		...
		cmd = utils.GoTool("kubectl", "delete", "ns", namespace)
		_, _ = utils.Run(cmd)
	})
	...
	AfterEach(func() {
		...
		if specReport.Failed() {
			...
			cmd := utils.GoTool("kubectl", "logs", controllerPodName, "-n", namespace)
			...
			cmd = utils.GoTool("kubectl", "get", "events", "-n", namespace, "--sort-by=.lastTimestamp")
			...
			cmd = utils.GoTool("kubectl", "logs", "curl-metrics", "-n", namespace)
			...
			cmd = utils.GoTool("kubectl", "describe", "pod", controllerPodName, "-n", namespace)
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := utils.GoTool("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
			...
			cmd = utils.GoTool("kubectl", "get", "service", metricsServiceName, "-n", namespace)
			_, err = utils.Run(cmd)
			...
			verifyMetricsEndpointReady := func(g Gomega) {
				cmd := utils.GoTool("kubectl", "get", "endpoints", metricsServiceName, "-n", namespace)
				output, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
			...
			cmd = utils.GoTool("kubectl", "run", "curl-metrics", "--restart=Never",
				"--namespace", namespace,
			...
			verifyCurlUp := func(g Gomega) {
				cmd := utils.GoTool("kubectl", "get", "pods", "curl-metrics",
...
func serviceAccountToken() (string, error) {
	...
	verifyTokenCreation := func(g Gomega) {
		...
		cmd := utils.GoTool("kubectl", "create", "--raw", fmt.Sprintf(
